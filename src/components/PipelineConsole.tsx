...
// Shared keyset-paginated + virtualized candidate console. Used both as the
// global shortlist console (no roleId) and as the per-role pipeline tab
// (roleId set → role filter hidden, query scoped to that role). This is the
// single implementation that replaced the old unbounded listJobCandidates path.
...
export function PipelineConsole({ roleId }: { roleId?: string }) {
  ...
  // Role filter dropdown only shown in the global console.
  ...
  const serverFilters = useMemo(
    () => ({
      roleId: lockRole ? roleId : filters.roleId === ALL ? undefined : filters.roleId,
    ...
    [lockRole, roleId, filters.roleId, filters.stage, filters.status, filters.needsHumanOnly, debouncedSearch],
  );

  const infinite = useInfiniteQuery({
    queryKey: ["pipeline", serverFilters] as const,
  ...
  const rows: PageRow[] = useMemo(() => infinite.data?.pages.flatMap((p) => p.rows) ?? [], [infinite.data]);
  ...
  return (
    <div>
      ...
      <div className="flex flex-wrap items-end gap-3 rounded-lg border border-border bg-card p-4">
        ...
        <div className="ios-mono pb-2 text-[10px] text-muted-foreground">
      ...
      <div className="mt-6 grid gap-6 lg:grid-cols-[minmax(0,520px)_1fr]">
        {/* Virtualized table */}
        <div className="rounded-lg border border-border bg-card outline-none" tabIndex={0} onKeyDown={onKeyDown}>
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="mt-0.5 flex items-center gap-2 text-xs text-muted-foreground">
                        {!lockRole && <><span className="truncate">{role?.title ?? "—"}</span><span>·</span></>}
        ...
        <div className="rounded-lg border border-border bg-card p-6">
          {!selectedRow ? (
            ...
            <DetailPanel
              ...
              onRescored={() => {
                queryClient.invalidateQueries({ queryKey: ["pipeline-detail", selectedRow.id] });
                queryClient.invalidateQueries({ queryKey: ["pipeline"] });
              }}
            />
          )}
...
function DetailPanel({
  applicationId, detail, loading, reason, setReason, reasonRef, pending,
  onAdvance, onHire, onReject, onPool, onRescored,
}: {
  ...
  const rerunMut = useMutation({
    mutationFn: () => rerun({ data: { applicationId } }),
